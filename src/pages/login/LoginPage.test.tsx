...
describe('LoginPage', () => {
  it('이미 로그인되어 있으면 홈으로 리다이렉트한다', () => {
    mockedUseAuth.mockReturnValue({
      user: { id: 'user-001', name: '김도윤', permissions: [], scope: { warehouseIds: [] } },
      isLoading: false,
      can: () => true,
      login: vi.fn(),
      logout: vi.fn(),
  ...
  it('로그인 폼을 보여준다', () => {
    mockedUseAuth.mockReturnValue({ user: null, isLoading: false, can: () => false, login: vi.fn(), logout: vi.fn() })
  ...
  it('로그인 성공 시 홈으로 이동한다', async () => {
    ...
    mockedUseAuth.mockReturnValue({ user: null, isLoading: false, can: () => false, login, logout: vi.fn() })
