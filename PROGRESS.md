...
### 2026-09-23 (액터 모드 선택 UI를 사이드바 모달로 통합)

- 별도 `/select-mode` 페이지와 페이지 전용 UI를 제거하고 액터 모드 선택기를 `ActorModeModal`로 이동
- 로그인 직후에는 콘솔 셸 위에 닫을 수 없는 필수 선택 모달을 표시하고, 모드 선택 후에는 사이드바의
  `CURRENT MODE` 카드로 같은 모달을 다시 열 수 있도록 변경
- 기존 모드가 있는 전환 상황에서는 닫기·배경 클릭으로 현재 콘솔에 복귀 가능. 권한 없는 모드의
  잠금 표시, 비활성 선택, 권한 요청 안내는 그대로 유지
- 테스트 66개, 빌드, 린트 통과. Chrome 접근성 트리에서 로그인 후 별도 페이지 이동 없이 `/` 위에
  모달이 표시되고 무권한/사용 가능 모드가 구분되는 것을 확인

### 2026-09-23 (액터 모드 라우팅 가드 및 모드 전환)

- 이슈 #30, `feat/actor-mode-routing-switch` 브랜치에서 `RequireActorMode` 가드 구현. 선택 모드가
  없거나 서버의 최신 사용자 허용 목록에 없는 상태로 하위 화면에 접근하면 선택 모달이 표시되는
  콘솔 홈으로 이동하고, 현재 모드에서 제공하지 않는 하위 화면 URL 직접 접근도 모드 홈으로 차단
- 모드별 내비게이션 구성 추가. 창고 관리자/본사 재고 담당자는 기존 대시보드·재고·평면도를
  재사용하고, 계정 관리자/MD는 각 업무에 맞는 독립 콘솔 홈 골격과 대시보드 메뉴만 표시
- 사이드바의 `CURRENT MODE` 카드를 눌러 중앙 모달에서 다른 허용 모드를 선택하도록 구성
- 전체 테스트, 빌드, 린트 통과

### 2026-09-23 (액터 모드 선택 모달과 잠금 상태)

- 이슈 #31, `feat/actor-mode-selector` 브랜치에서 액터 모드 선택 UI 구현
- 등록된 네 모드를 모두 카드로 표시하고 사용자의 허용 목록에 없는 모드는 숨기지 않고 잠금 배지,
  비활성 버튼, 권한 요청 안내를 함께 표시. 비활성 버튼과 `aria-describedby`로 키보드·스크린리더에서도
  선택 불가 상태를 전달
- 데모 사용자는 창고 관리자와 본사 재고 담당자만 선택할 수 있도록 mock 응답 구성. 로그인 후
  필수 선택 모달에서 허용된 모드를 선택하면 콘솔 홈을 사용할 수 있음
- 전체 테스트 60개, 빌드, 린트 통과. 실제 Chrome에서 로그인→잠금 모드 확인→창고 관리자 선택→
  대시보드 진입까지 확인

### 2026-09-23 (액터 모드 도메인 및 선택 세션)

- 이슈 #32, `feat/actor-mode-domain-session` 브랜치에서 계정 관리자/MD/창고 관리자/본사 재고
  담당자 모드의 타입과 메타데이터 레지스트리 구현
- `/api/auth/me` 응답에 사용자의 허용 모드 목록을 추가하고 Zod로 검증. 선택 모드는 새로고침에는
  유지되지만 새 로그인에는 이어지지 않도록 `sessionStorage`에 저장
- 모드 선택 API가 현재 사용자의 허용 목록을 검사해 권한 없는 모드 선택을 거절하고, 서버의 최신
  허용 목록에서 빠진 기존 선택값과 로그아웃·인증 만료 시 선택값을 초기화하도록 구성
- 후속 이슈를 선택 화면/잠금 UI(#31), 모드별 라우팅·전환(#30)으로 분리
