...
- 이슈 #30, `feat/actor-mode-routing-switch` 브랜치에서 `RequireActorMode` 가드 구현. 선택 모드가
  없거나 서버의 최신 사용자 허용 목록에 없는 모드이면 `/select-mode`로 이동하고, 현재 모드에서
  제공하지 않는 하위 화면 URL 직접 접근은 모드 홈으로 차단
- 모드별 내비게이션 구성 추가. 창고 관리자/본사 재고 담당자는 기존 대시보드·재고·평면도를
  재사용하고, 계정 관리자/MD는 각 업무에 맞는 독립 콘솔 홈 골격과 대시보드 메뉴만 표시
- 사이드바에 현재 모드와 `모드 전환` 버튼을 추가. 전환 시 기존 선택을 지우고 모드 선택 화면으로
  돌아가 다른 허용 모드를 선택하도록 구성
- 전체 테스트 65개, 빌드, 린트 통과. 실제 Chrome에서 선택한 창고 관리 모드 표시와 사이드바의
  모드 전환 버튼이 `/select-mode`로 이동하는 것까지 확인

### 2026-09-23 (액터 모드 선택 화면과 잠금 상태)

- 이슈 #31, `feat/actor-mode-selector` 브랜치에서 로그인 직후 진입하는 `/select-mode` 화면 구현
- 등록된 네 모드를 모두 카드로 표시하고 사용자의 허용 목록에 없는 모드는 숨기지 않고 잠금 배지,
  비활성 버튼, 권한 요청 안내를 함께 표시. 비활성 버튼과 `aria-describedby`로 키보드·스크린리더에서도
  선택 불가 상태를 전달
- 데모 사용자는 창고 관리자와 본사 재고 담당자만 선택할 수 있도록 mock 응답 구성. 로그인 후 항상
  모드 선택 화면으로 이동하고 허용된 모드를 선택하면 콘솔 홈으로 이동
