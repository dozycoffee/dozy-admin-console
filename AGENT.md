...
DOZY COFFEE 본사와 창고 운영을 위한 통합 관리자 콘솔이다. 역할이 다른 사용자(본사 MD, 창고
운영자 등)가 같은 화면을 공유하면서, 권한에 따라 조회·수정 가능한 범위만 달라지는 단일 콘솔을
지향한다.
...
### 권한 모델 — 화면 복제 대신 Permission + AccessScope

역할별로 화면이나 라우트를 복제하지 않는다. 모든 사용자가 동일한 메뉴·화면을 사용하되, 다음 두
축으로 접근을 제어한다.
